      ...
      },
      {
        ...
        guidelines: [
          ...
          },,
          {
            ...
            category: "RT & SACT Toxicities",
            summary: "Immediate management of chemotherapy extravasation. Vesicant vs irritant classification, stop-aspirate-elevation protocol, antidote administration (DMSO for anthracyclines, hyaluronidase for vinca alkaloids), cold/warm compress guidance, plastic surgery referral thresholds, and documentation requirements.",
            tags: ["Extravasation", "Vesicant", "Anthracycline", "DMSO", "Vinca alkaloids"],
          ...
          {
            id: "onco-io-toxicity",
            title: "Immunotherapy Toxicity (irAE)",
            category: "RT & SACT Toxicities",
            summary: "Grading and organ-specific management of immune-related adverse events: pneumonitis (CT criteria, steroid initiation), colitis (endoscopy indications, infliximab), hepatitis (LFT thresholds), endocrinopathies (hypothyroidism, adrenalitis, diabetes insipidus), and neurotoxicity. Checkpoint inhibitor hold/restart criteria.",
            tags: ["irAE", "Pneumonitis", "Colitis", "Infliximab", "Steroids"],
            related: ["onco-neutropenic-sepsis", "onco-extravasation"],
            pdfUrl: "https://www.nuh.nhs.uk/guidelines/io-toxicity",
            portalUrl: "https://clinicalportal.nuh.nhs.uk/io-toxicity",
            updated: "Apr 2025",
          },,
          {
            id: "onco-rt-toxicity",
            ...
            related: ["onco-io-toxicity", "onco-neutropenic-sepsis"],
          ...
          },,
          {
            ...
            related: ["onco-io-toxicity", "onco-pericardial-tamponade"],
...
id: "immunotherapy",
    ...
    calculators: [
      { label: "irAE Grading Reference", url: "#calc-irae" },
      { label: "Steroid Dose Calculator", url: "#calc-steroid" },
      { label: "Cortisol / Synacthen Interpreter", url: "#calc-cortisol" },
    ],
    subsites: [
      {
        id: "io-skin",
    ...
    // Flatten all subsite guidelines for search/nav
    get guidelines() {
      return this.subsites.flatMap(ss => ss.guidelines);
    },
  },
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar-pin-btn:hover { color: var(--accent); border-color: var(--accent); background: var(--accent-light); }
  .sidebar-pin-btn.active { color: var(--accent); border-color: var(--accent); background: var(--accent-light); }
...
export default function App() {
  ...
  const [favourites, setFavourites] = useState(() => {
    try { return JSON.parse(localStorage.getItem("cg_favs") || "[]"); } catch { return []; }
  });
  ...
  useEffect(() => {
    try { localStorage.setItem("cg_favs", JSON.stringify(favourites)); } catch {}
  }, [favourites]);
  ...
  const navigate = (v) => {
    ...
    if (!sidebarPinned || mobile) setSidebarOpen(false);
  };

  const openSidebar = () => setSidebarOpen(true);
  const closeSidebar = () => { if (!sidebarPinned) setSidebarOpen(false); };
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {/* Sidebar */}
        <aside className={`sidebar ${!sidebarOpen ? "collapsed" : ""} ${!sidebarPinned && sidebarOpen ? "floating" : ""}`}>
...
function HomePage({ sites, favourites, onStar, onNavigate }) {
    ...
    <div>
      ...
      <div className="favourites-section">
        <div className="home-section-title" style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ color: "#f59e0b" }}><IconStar filled /></span>
          Favourites
        </div>
        {favGuides.length === 0 ? (
          <p className="fav-empty">Star any guideline to pin it here.</p>
        ) : (
          <div className="favourites-grid">
            {favGuides.map(g => {
              const site = SITES.find(s => s.id === g.siteId);
              return (
                <div key={g.id} className="fav-chip"
                  style={{ "--site-color": site.color, "--site-accent": site.accent }}
                  onClick={() => onNavigate({ type: "guideline", guidelineId: g.id })}>
                  <span className="fav-chip-dot" style={{ background: site.color }} />
                  {g.title}
                </div>
              );
            })}
